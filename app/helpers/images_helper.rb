module ImagesHelper
    ...
    mmName = "millimeter";
    pxName = "pixel";
    mmUppercaseName = "Millimeter";
    pxUppercaseName = "Pixel";

    pixel_unit = 0.26458333331386;      #mm per pixel
    mm_unit = 3.7795275593333;          #pixel per mm
    defaultDPI = 600;                   # DPI for print versions

    DefaultPath = "/Uploads/GeneratedProducts/"
    DefaultExtension = ".jpg"
    def self.ImagePath(fileName)        
        sPath = DefaultPath + Time.new.strftime("%Y%m%d%H%M%S")
                
        #unless File.directory?(sPath)
        #  FileUtils.mkdir_p(sPath)
        #end
        Dir.mkdir(sPath) unless Dir.exists?(sPath) #make folder if not exists

        sPath += DefaultExtension
    end
    def self.DefaultImageExtension
        ".jpg"
    end
    #generate Name folder
    def self.folderName
        SecureRandom.uuid
    end
    #Convert px to mm
    def pixel2mm(pixel_value)
    ...
    def hello
        ...
        imagePrint = MiniMagick::Image.new('mask.png')
